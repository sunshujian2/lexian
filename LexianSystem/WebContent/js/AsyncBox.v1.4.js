...
var asyncboxClone  = $.extend(true,{},asyncbox);

asyncbox.open =  function(option){
	if(!option){
		throw  Error("must setting a id or url");
		return;
	}
	var id = option.id;
    if(id){
	...
	if(!asyncbox.data[id]){
		...
		if(jqueryObject.length==0){
			throw Error("don't have this jquery object");
			return;
		}
		asyncbox.data[id]  = jqueryObject.html();
		jqueryObject.remove();
	}
	option.html = asyncbox.data[id];
	asyncboxClone.open(option);
	if(option.loadData&&typeof option.loadData=="function"){
		option.loadData($("#"+id+"_content"));
	}
    }
    else{
    	asyncboxClone.open(option);
